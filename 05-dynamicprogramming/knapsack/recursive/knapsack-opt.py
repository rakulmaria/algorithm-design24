...
def solve():
    ...
    def backtrack(row, col):
        ...
        while row > 0 and col > 0:
            # if the value at this memory is different than the one directly above it, we took this item
            if memory[row][col] != memory[row-1][col]:
                # store the index to the result
                res.append(row-1)
                # C - weight of item
                col -= items[row][1]
            row -= 1
    ...
    while line:
        ...
        memory = [[0 for _ in range(C+1)] for _ in range(n+1)]
